...
describe('Dispatcher', () => {
    it('dispatches action to the correct plugin', async () => {
        ...
        const plugin = definePlugin({
            ...
            version: '1.0.0',
            actions: {
                'ticket.created': async () => { called.value = true; },
            },
        });
    ...
    it('calls webhook handler', async () => {
        const plugin = definePlugin({
            ...
            version: '1.0.0',
            webhooks: {
                'POST /events': async (_ctx, req) => ({ received: true }),
            },
        });
        ...
        const result = await dispatcher.callWebhook('test', 'POST', '/events', {
            body: { type: 'test' }, params: {}, query: {}, headers: {},
        });
    ...
    it('skips timed-out filter but continues chain', async () => {
        const slow = definePlugin({
            ...
            version: '1.0.0',
            filters: {
                'channels': { priority: 10, handler: async (v: string[]) => {
