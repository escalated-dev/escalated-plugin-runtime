...
export function createContextProxy(pluginName: string, sendRequest: RpcSender): PluginContext {
    ...
    return {
        ...
        broadcast: {
            toChannel: (channel, event, data) =>
                rpc('ctx.broadcast.toChannel', { channel, event, data }) as Promise<void>,
            toUser: (userId, event, data) =>
                rpc('ctx.broadcast.toUser', { userId, event, data }) as Promise<void>,
            toTicket: (ticketId, event, data) =>
                rpc('ctx.broadcast.toTicket', { ticketId, event, data }) as Promise<void>,
        },
        log: {
            info: (message, data) => { rpc('ctx.log', { level: 'info', message, data }).catch(() => {}); },
            warn: (message, data) => { rpc('ctx.log', { level: 'warn', message, data }).catch(() => {}); },
            error: (message, data) => { rpc('ctx.log', { level: 'error', message, data }).catch(() => {}); },
            debug: (message, data) => { rpc('ctx.log', { level: 'debug', message, data }).catch(() => {}); },
        },
        tickets: {
